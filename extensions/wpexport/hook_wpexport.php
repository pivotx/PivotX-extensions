...
THEEND;
    ...
    private static function outputWXR_Chapters($chapter)
    {
        ...
        $output .= '<!-- Item for post_id ' . $record['post_id'] . ' -->'."\n";
...
THEEND;
    ...
    private static function convertPageToItem($page, $comments)
    {
        ...
        if (array_key_exists($item['chapter'], $chaparray)) {
            ...
            $item['post_parent'] = '0'; // added by harm: put it in convert routine for chapter option on pages
        }
        return $item;
    }
    ...
    private static function outputWXR_Items(&$data, $comments, $callback)
    {
        ...
        global $WPEXPORT;
        $output = '';
        $parse = isset( $_GET['parse'] ) ? $_GET['parse'] : '';   // added by Harm
        foreach($data as &$record) {
            ...
            // @todo
            // - add categories in <item>
            // harm: this appears to already been done.....?
...
//@@CHANGE REPLACE STRINGS HERE -- end
            ...
            $excerpt_encoded = ''; // parse_intro_or_body($record['introduction']); // xiao: removed, because redundant and too much data
            // harm: kept the excerpt encoded empty because WP also exports it like that

            if ($parse != 'no') {
                $content_encoded = parse_intro_or_body($record['introduction']); // xiao: was 'body'
                $content_encoded .= parse_intro_or_body($record['body']); // harm: 'body' added
            } else {
                // added by Harm
                $content_encoded = $record['introduction'];
                $content_encoded .= $record['body'];
            }
            $content_encoded = html_entity_decode($content_encoded, ENT_QUOTES, "UTF-8");   // added by Harm
    ...
    public static function exportEntries()
    {
        ...
        //$output .= self::outputWXR_Categories();   // Harm: not needed
        $output .= self::outputWXR_Tags();
...
function pageWpexport()
{
    ...
    //       they cannot
    //       These ids can also be used in the chaparray after importing the chapters and exporting the pages
